...
const templateWithJQuery = () => {
    starWarsPeople.forEach((element,index) => {
        var cloneMe = $('#template').clone();
        console.log(index);
        
        cloneMe.children('h2').text(element.name);
        cloneMe.children('h3').text(element.height);
        cloneMe.children('p').text(element.eye_color);
        $('main').append(cloneMe);
      });
...
arr.forEach(element =>{
  let test = element.slice(element.indexOf( ' ', element.indexOf( ' ' ) +1)+1);
  result.push(test)
});
return result
};
