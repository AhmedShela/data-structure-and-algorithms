...
function getCurrentEvents(request, response){
  ...
  const mapCurrentEvents = () => {
      let myarr = currentEvents.news.map(item =>{
          return new MyConst(item);
      })
      return myarr;
    // Solution code here...
  }
...
const returnNames = (arr) => {
  // Solution code here...
  let myArr =arr.reduce((acc,val,idx)=>{
    acc.push(val.name);
     return acc;
...
const reversedString = (myStr) => {
  // Solution code here...
  let nstr=[];
for(let i in myStr){
  nstr.unshift(myStr[i])
}
  console.log(nstr);

   let stt =nstr.reduce((acc,val,idx)=>{
     acc=acc+ val
      return acc;
